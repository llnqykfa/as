package com.aixianshengxian.activity.returned;
...
import com.aixianshengxian.adapters.ReturnedUcodeAdapter;
...
public class ReturnedActivity extends BaseActivity implements View.OnClickListener {
    ...
    private RecyclerView returned_list;
    private ReturnedUcodeAdapter mAdapter;
    ...
    protected void initViews() {
        ...
        tv_head_title.setText("还筐");
        btn_returned = (Button) findViewById(R.id.btn_returned);
    ...
    protected void initEvents() {
        ...
        btn_returned.setOnClickListener(this);
        tv_delete.setOnClickListener(this);
        iv_delete.setOnClickListener(this);

        mData = new ArrayList<>();
        SP = SPUtil.contains(ReturnedActivity.this,"UCODE_RETURNED_KEY");
        if(SP == true) {
            String str = (String) SPUtil.get(ReturnedActivity.this, "UCODE_RETURNED_KEY", "");
            if(str != null){
                mData = gson.fromJson(str, new TypeToken<List<UcodeMessage>>(){}.getType());
            }
        }

        returned_list = (RecyclerView) findViewById(R.id.returned_listview);
        LinearLayoutManager layoutManager = new LinearLayoutManager(this);
        layoutManager.setOrientation(LinearLayoutManager.VERTICAL);
        returned_list.setLayoutManager(layoutManager);
        mAdapter = new ReturnedUcodeAdapter(mData);//将数据传入适配器
        returned_list.setAdapter(mAdapter);//完成适配器设置
    }
        ...
        switch (v.getId()) {
            case R.id.image_personal:
                if (mData.size() != 0) {
                    ...
                    SPUtil.put(ReturnedActivity.this,"UCODE_RETURNED_KEY",jsonStr);
                }
    ...
    private void RefreshData() {
        ...
        if(uCode.equals("")){
            ...
            //Toast.makeText(ReturnedActivity.this,"请先扫描标签",Toast.LENGTH_SHORT).show();
        } else {
            ...
            /*if(mData.size() == 0){//判断数据的长度是否为0
                ...
                showCustomToast("还筐成功");
                Toast.makeText(ReturnedActivity.this,"还筐成功",Toast.LENGTH_SHORT).show();
            } else {
                int total = 0;//通过计算total判断标签是否被扫描过
                for(int i = 0;i < mData.size();i ++){
                    if(uCode.equals(mData.get(i).getUCode())){
                        showCustomToast("这个标签被扫描过");
                        Toast.makeText(ReturnedActivity.this,"这个标签被扫描过",Toast.LENGTH_SHORT).show();
                        total = total + 1;
                    } else {
                        total = total + 0;
                    }
    ...
    private void Delete() {
        SPUtil.remove(ReturnedActivity.this,"UCODE_RETURNED_KEY");
        mData.clear();
        mAdapter.notifyDataSetChanged();
    }

    public void setResultAlertDialog(String response) {
        final AlertDialog alert = new AlertDialog.Builder(ReturnedActivity.this).create();
        alert.setTitle("提示信息");
    ...
    private void getUcodeResult (final String uCode,final String operateTime) {
        ...
        OkHttpUtils.postString().url(UrlConstants.URL_RETURNED)
