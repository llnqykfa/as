package com.aixianshengxian.activity.unable;
...
public class UnableActivity extends BaseActivity implements View.OnClickListener {
    private ImageView image_personal,iv_delete;
    private TextView tv_head_title,tv_delete;
    private Button btn_unable;
    private EditText edt_code;
    private RecyclerView unable_listview;
    ...
    protected void initViews() {
        ...
        tv_head_title.setText("报废");
        btn_unable = (Button) findViewById(R.id.btn_unable);
        tv_delete = (TextView) findViewById(R.id.tv_delete);
        edt_code = (EditText) findViewById(R.id.edt_code);
        iv_delete = (ImageView) findViewById(R.id.iv_delete);

        mData = new ArrayList<>();
        SP = SPUtil.contains(UnableActivity.this,"UCODE_UNABLE_KEY");
        if(SP == true) {
            String str = (String) SPUtil.get(UnableActivity.this, "UCODE_UNABLE_KEY", "");
            if(str != null){
                mData = gson.fromJson(str, new TypeToken<List<UcodeMessage>>(){}.getType());
            }
        }

        unable_listview = (RecyclerView) findViewById(R.id.unable_listview);
        LinearLayoutManager layoutManager = new LinearLayoutManager(this);
        layoutManager.setOrientation(LinearLayoutManager.VERTICAL);
        unable_listview.setLayoutManager(layoutManager);
        mAdapter = new ReceiveUcodeAdapter(mData);//将数据传入适配器
        unable_listview.setAdapter(mAdapter);//完成适配器设置
    }
        ...
        switch (v.getId()) {
            case R.id.image_personal:
                if (mData.size() != 0) {
                    ...
                    SPUtil.put(UnableActivity.this,"UCODE_UNABLE_KEY",jsonStr);
                }
    ...
    private void Delete() {
        SPUtil.remove(UnableActivity.this,"UCODE_RUNABLE_KEY");
        mData.clear();
        mAdapter.notifyDataSetChanged();
    }

    public void setResultAlertDialog(String response) {
        final AlertDialog alert = new AlertDialog.Builder(UnableActivity.this).create();
        alert.setTitle("提示信息");
    ...
    private void getUcodeResult (final String uCode,final String operateTime) {
        ...
        OkHttpUtils.postString().url(UrlConstants.URL_UNABLE )
